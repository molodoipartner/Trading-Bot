...
for (let i = 0; i < candles.length; i++) {
    ...
    if (hh === "23") continue;
*/
    if (day === 6) continue; // ❌ пропускаем выходные
...
*/
    ...
    let dynamicTP;
    let dynamicSL;
    let dynamicVolumeMultiplier;
    
    if (cb < 10) {
      dynamicTP = 0.0085;
      dynamicSL = 0.0016;
      dynamicVolumeMultiplier = 1;
    } else if (cb < 14) {
      dynamicTP = 0.0090;
      dynamicSL = 0.0021;
      dynamicVolumeMultiplier = 1;
...
//dynamicSL = 0.0022 0.0086 not bad
    const addPrice2 = entryPrice1 * (1 - dynamicSL); // 0.9% от первой цены, сюда ставим 2-ю позицию
    ...
    */
    ...
    for (let j = i + 1; j < candles.length; j++) {
      ...
      */
      ...
      if (secondOpened) {
        ...
        // ===== СРЕДНИЙ ВХОД =====
        const totalMoney =
          (VOLUME1 * dynamicVolumeMultiplier) +
          VOLUME2 +
        ...
        const totalAsset =
          (VOLUME1 * dynamicVolumeMultiplier) / entryWithSpread1 +
    ...
    trades.push({
      ...
      phase: "GRID",
      result: exitReason,
      volume: (VOLUME1 * dynamicVolumeMultiplier),
      profitQuoted: profit(exitWithSpread, entryWithSpread1, (VOLUME1 * dynamicVolumeMultiplier)),
      spreadUsed: SPREAD,
      positionNumber: 1,
      volumessum: volumessum,
      result2: profit(exitWithSpread, entryWithSpread1, (VOLUME1 * dynamicVolumeMultiplier)) > 0 ? "WIN" : "LOSS",

...
const isHaveWeCollectedLiquidity = (candles, currentIndex) => {
  ...
  const isSweep = prevCandle.low < lowestLow;
  const isReclaim = prevCandle.close > lowestLow;

  const isConfirmed =
    currentCandle.close > lowestLow &&
    currentCandle.low > lowestLow;

  const canbeopened = isSweep && isReclaim && isConfirmed;
...
📊 PREV CANDLE:
time:  ${new Date(prevCandle.time).toLocaleString()}
low:   ${prevCandle.low}
...
🧠 CONDITIONS:
sweep:     ${prevCandle.low} < ${lowestLow} → ${isSweep}
reclaim:   ${prevCandle.close} > ${lowestLow} → ${isReclaim}
confirmed: close>level & low>level → ${isConfirmed}
...
const changePercent = 1;
    const { canbeopened, liquidityLevel } = isHaveWeCollectedLiquidity(candles, i);
